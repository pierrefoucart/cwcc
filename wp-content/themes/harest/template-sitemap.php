...
get_header(); ?>
			...
			<div class="one_half last">
				<h3 class="page-title"><?php _e( 'Posts', 'harest'); ?>:</h3>
				<ul class="sitemap-posts">
					...
					foreach ( $cats as $cat ) {
					 ...
					 $loop = new WP_Query('posts_per_page=-1&cat='.$cat->cat_ID);
					  ...
					  echo '<li class="category"><a href="' . esc_url( get_category_link($cat->term_id) ) . '">' . __( 'Category:', 'harest' ) . ' ' . esc_html( $cat->cat_name ) . ' (' . esc_html( $cat->category_count ) . ')' . "\n";
				...
				<h3 class="page-title"><?php _e( 'Archives', 'harest'); ?>:</h3>
